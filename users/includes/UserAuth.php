...
class UserAuth {
    ...
    public function login($email, $password) {
        try {
            ...
            $requireVerification = getSetting('require_email_verification', 'yes');
            if ($requireVerification === 'yes' && !$user['email_verified']) {
    ...
    public function register($email, $password, $name = null) {
        try {
            ...
            // Get next user ID
            $lastUserId = $this->db->getValue("SELECT MAX(CAST(user_id AS UNSIGNED)) FROM site_users WHERE user_id REGEXP '^[0-9]+$'");
            $nextUserId = $lastUserId ? $lastUserId + 1 : 100001;
