...
/**
 * Get active tutorials for homepage display
 */
function getActiveTutorials($limit = 5) {
    try {
        $db = Database::getInstance();
        return $db->getRows(
            "SELECT * FROM tutorials_ads 
        ...
        error_log("getActiveTutorials error: " . $e->getMessage());
        return [];
    }
}
 ...
 */
function getAllTutorials($activeOnly = false) {
    try {
        $db = Database::getInstance();
        
        $sql = "SELECT * FROM tutorials_ads";
        if ($activeOnly) {
            $sql .= " WHERE status = 'active'";
        }
        ...
        error_log("getAllTutorials error: " . $e->getMessage());
        return [];
    }
}
