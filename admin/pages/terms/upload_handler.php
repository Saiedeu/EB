<?php
// upload_handler.php for pages with TinyMCE
session_start();
...
// Get upload directory path - store in /assets/uploads/media
...
// Allowed file types
$allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'mp4', 'webm', 'ogg'];
$maxFileSize = in_array($fileExtension, ['mp4', 'webm', 'ogg']) ? 10 * 1024 * 1024 : 5 * 1024 * 1024; // 10MB for videos, 5MB for images
...
if ($fileSize > $maxFileSize) {
    ...
    echo json_encode([
        'success' => false, 
        'message' => 'File too large (' . round($fileSize / 1024 / 1024, 2) . 'MB). Max size: ' . round($maxFileSize / 1024 / 1024, 1) . 'MB'
    ]);
    exit;
}
...
if (function_exists('finfo_open')) {
    ...
    $mimeType = $isVideo ? 'video/' . $fileExtension : 'image/' . ($fileExtension === 'jpg' ? 'jpeg' : $fileExtension);
}

// Generate unique filename with consistent prefix
$newFileName = 'pages_tinymce_' . time() . '_' . rand(1000, 9999) . '.' . $fileExtension;
...
$fileUrl = SITE_URL . '/assets/uploads/media/' . $newFileName;
...
try {
    ...
    $mediaData = [
        ...
        'file_path' => 'assets/uploads/media/' . $newFileName, // Store relative path
