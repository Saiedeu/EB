<?php
/**
 * Exchange Bridge - Main Entry Point
 * 
...
// CRITICAL: Initialize protection system FIRST
// Start session
session_start();
...
// Initialize security class (if not already done by protector)
if (!isset($GLOBALS['eb_security'])) {
    $security = Security::getInstance();
} else {
    $security = $GLOBALS['eb_security'];
}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            try {
                ...
                $activeTutorials = $db->getRows(
                    "SELECT * FROM tutorials_ads WHERE status = 'active' AND display_on_homepage = 1 ORDER BY priority ASC, created_at DESC LIMIT 3"
                );
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="tutorial-item bg-gray-50 dark:bg-gray-700 rounded-lg overflow-hidden shadow-sm">
                            <?php if ($tutorial['type'] === 'video_upload' && $tutorial['file_path']): ?>
                                <!-- Uploaded Video -->
                                <video class="w-full h-48 object-cover" controls poster="<?php echo $tutorial['thumbnail'] ? ASSETS_URL . '/uploads/tutorials/' . $tutorial['thumbnail'] : ''; ?>">
                                    <source src="<?php echo ASSETS_URL . '/uploads/tutorials/' . htmlspecialchars($tutorial['file_path']); ?>" type="video/mp4">
                            ...
                            <?php elseif ($tutorial['type'] === 'image_upload' && $tutorial['file_path']): ?>
                                ...
                                <div class="w-full h-48 bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                    <img src="<?php echo ASSETS_URL . '/uploads/tutorials/' . htmlspecialchars($tutorial['file_path']); ?>" 
                            ...
                            <?php elseif ($tutorial['type'] === 'youtube_embed' && $tutorial['embed_code']): ?>
                                ...
                                <div class="w-full h-48">
                                    ...
                                    <iframe class="w-full h-full" 
                                            src="https://www.youtube.com/embed/<?php echo $matches[1]; ?>?rel=0" 
                                            frameborder="0" 
                                            allowfullscreen>
                                    </iframe>
                                    <?php else: ?>
                            ...
                            <?php elseif ($tutorial['type'] === 'facebook_embed' && $tutorial['embed_code']): ?>
                                ...
                                <div class="w-full h-48">
                                    <iframe class="w-full h-full" 
                                            src="https://www.facebook.com/plugins/video.php?href=<?php echo urlencode($tutorial['embed_code']); ?>&show_text=false&width=560" 
                                            frameborder="0" 
                                            allowfullscreen>
                                    </iframe>
                                </div>
                            ...
                            <?php elseif ($tutorial['type'] === 'google_drive_embed' && $tutorial['embed_code']): ?>
                                ...
                                <div class="w-full h-48">
                                    ...
                                    ?>
                                    <iframe class="w-full h-full" 
                                            src="https://drive.google.com/file/d/<?php echo $matches[1]; ?>/preview" 
                                            frameborder="0">
                                    </iframe>
                                    <?php else: ?>
                                    <div class="w-full h-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                        <p class="text-gray-500">Invalid Google Drive URL</p>
                                    </div>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="p-4">
                                <h3 class="font-semibold text-sm mb-2"><?php echo htmlspecialchars($tutorial['title']); ?></h3>
                                <?php if ($tutorial['description']): ?>
                                <p class="text-xs text-gray-600 dark:text-gray-400"><?php echo htmlspecialchars($tutorial['description']); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endforeach; ?>
            ...
            <?php 
                ...
                error_log("Error loading tutorials: " . $e->getMessage());
            }
            ?>
        </div>
...
.tutorial-item .video-container iframe,
.tutorial-item .video-container video {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // License protection monitoring
    setInterval(function() {
        // Check for tampering attempts
        if (typeof ebPeriodicSecurityCheck === 'function') {
